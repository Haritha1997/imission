...
function addnewinstnametoipsecconfig(id,slnumber) {
	
	var table = document.getElementById('WiZConf');
	var rowcnt = table.rows.length;
	if(rowcnt==2)
	{
		alert("Maximum 1 rows are allowed in IPSEC Table");
		return false;
	}
	...
	var url="savepage.jsp?page=ipsec_autofb&slnumber="+slnumber+"&instancename="+newinstname+"&action=add";
	openInFrame(url);
}
...
function gototunnelConfigpage(instancename,slnumber) {
	var instname = document.getElementById(instancename).value;
	var url="tunnelconfig.jsp?instancename="+instname+"&slnumber="+slnumber;
	openInFrame(url);
}

function addInstancenames(id, instancename, enable, optlevel, interface, status, uptime) {
	...
	document.getElementById("interface" + id).value = interface;
...
function deleteRow(rowid,insnameid,slnumber) 
{
	...
	var url="savepage.jsp?page=ipsec_autofb&slnumber="+slnumber+"&instancename="+instname+"&action=delete";
...
function showErrorMsg(errormsg)
{
   ...
   <body>
      ...
      <form method="post" action="savepage.jsp?page=ipsec_autofb&slnumber=<%=slnumber%>&action=apply">
	  <input type="text" id="ipsecrwcnt" name="ipsecrwcnt" value="1" hidden="">
         <blockquote>
            <p align="center" class="style1">IPSEC with Autofallback</p>
         </blockquote>
         ...
         <table align="center" id="WiZConf1">
            <tbody>
               <tr>
                  ...
                  <td colspan="2"><button type="button" class="button1" id="add" value="Add" onclick="addnewinstnametoipsecconfig('nwinstname','<%=slnumber%>');">Add</button></td>
               </tr>
            </tbody>
         </table>
   ...
   <% 
            	   for(int j=0;j<ipsecnumarr.size();j++)
            	   {
            		    ...
            		    String enable = ipsecobj.getString("Activation").replace("Disable","No")==null? "":ipsecobj.getString("Activation").replace("Disable","No");
                  		String prfpeer = ipsecobj.getString("DualPeer").replace("Disable","Single Peer")==null? "":ipsecobj.getString("DualPeer").replace("Disable","Single Peer");
                   ...
                   <script type="text/javascript">
	 				 addRow('WiZConf');
	 				addTunnelData(<%=(j+1)%>,'<%=instancename%>','<%=enable%>','<%=prfpeer%>','<%=lclcptepnt%>','<%=status%>');
	 			 </script>
				   <%}%>
