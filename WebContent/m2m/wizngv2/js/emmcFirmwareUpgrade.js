...
function firmwareSelected() {
	...
	if (fileName.split('.').pop() == "ext4")
		maxSize = 101;
	else if (fileName.split('.').pop() == "md5")
		maxSize = 1;
...
function firmwareUpload() {
	...
	if (!md5File) {
		firmwareUploadSelectFile("Upload a md5 file ...", ".md5")
		return;
	} 
	if (!ext4File) {
		firmwareUploadSelectFile("Upload a ext4 file ...", ".ext4")
		return;
	} 
...
function firmwareUploadComplete(evt) {
	...
	if (evt.target.readyState == 4 && evt.target.status == 200) {
		...
		if (currentFile.split('.').pop() == "ext4")
			ext4File = currentFile;
		else if (currentFile.split('.').pop() == "md5")
			md5File = currentFile;
...
//                        $("html").html(responseText);
	...
	xhr.send('ext4File='+ext4File+'&md5File='+md5File+'&backup='+backup);
}
