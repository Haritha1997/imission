...
{ 
	...
	{ 
		...
		"<image style=\"cursor:pointer\" id=\"delrw"+iprows+"\" name=\"delrw"+iprows+"\" src=\"images/delete.png\" width=\"22\" height=\"22\" align=\"right\" title=\"Delete\" onclick=\"deleteRow('portrow"+iprows+"');\"></image></td>"+
		"<td hidden>0</td>"+
		"<td hidden>"+iprows+"</td>"+
		"</tr>"; 
		$('#WiZConff').append(row);  
...
function gotoforwardeditpage(showinstancename)
{
	...
	//var slnumber = document.getElementById("slno").value;
	 location.href = "edit_gre6in4.jsp";
	//location.href = "Nomus.cgi?cgi=gre6in4EditInstance="+instname+".cgi";
}
...
function addforwardeditpage(showinstancename, addinstance)
{
	...
	var instname = document.getElementById(showinstancename).value;
	 ...
	 location.href = "edit_gre6in4.jsp";
	//location.href = "Nomus.cgi?cgi=gre6in4AddInstance="+instname+".cgi";
}

function deleteforwardeditpage(id)
{
...
function validateGre()
		{
			...
			valid = validateIPv6("lcl_ipv6_address",true,"Local Gre IPv6 Address");
			if (!valid) {
				if (lclipaddress.value.trim() == "") 
					alertmsg += "Local Gre IPAddresss should not be empty\n";
				else 
					alertmsg += "Local Gre IPAddresss is not valid\n";
			}

			valid = validateIPv6("rem_ipv6_ipaddress",true,"Remote Subnet IPV6 network");
			if (!valid) {
				if (rmtipaddress.value.trim() == "") 
					alertmsg += "Remote Subnet IPAddress should not be empty\n";
				else 
					alertmsg += "Remote Subnet IPAddress is not valid\n";
			}
