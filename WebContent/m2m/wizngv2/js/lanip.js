...
function addIPV6RowAndChangeIcon(rowid)
{
	...
	if(remove != null)
	remove.style.display ="inline";
	$("#ipconfig1").append("<tr id='ipv6"+ipv6rows+"'><td min-width=\"200\"><div>IPV6 Address</div></td><td width=\"195\"><div><input type='text' class='text' style='position: relative; left: 3px;display:inline block' id='IPV6"+ipv6rows+"' name='IPV6"+ipv6rows+"' onkeypress='return IPV6avoidSpace(event);' style='display:inline block;' onmouseover=\"setTitle(this)\" onfocusout=\"validateIPv6('IPV6"+ipv6rows+"',false,'IPV6 Address',false)\"></input><i class='fa fa-plus' id='adddns"+ipv6rows+"' style='font-size:10px; margin-left:7px;color:green;display:inline block'; onclick='addIPV6RowAndChangeIcon("+ipv6rows+")'></i><i class='fa fa-close' style='display:inline;font-size:10px; margin-left:5px; color:red;' id='removedns"+ipv6rows+"' onclick='deleteIPV6tableRow("+ipv6rows+")'></i><input hidden id='row"+ipv6rows+"' value='"+ipv6rows+"'></div></td></tr>");
