...
{ 
	...
	{ 
		...
		"<td><button type=\"button\" id=\"editrw"+iprows+"\" name=\"editrw"+iprows+"\" class=\"button1\" align=\"left\" onclick=\"gotoNATRuleEditPage('instancename"+iprows+"','"+version+"')\">Edit <i class='fas fa-edit' style='font-size:12px;color:white'></i></button>"+
		"<image style=\"cursor:pointer;margin-left:10px;margin-bottom:10px;\" id=\"delrw"+iprows+"\" name=\"delrw"+iprows+"\" src=\"images/delete.png\" width=\"22\" height=\"22\" align=\"middle\"  title=\"Delete\" onclick=\"deleteNATRulepage('instancename"+iprows+"','"+slnumber+"','"+version+"');\"></image></td>"+
		"<td hidden>0</td>"+
		"<td hidden>"+iprows+"</td>"+
		"</tr>"; 
		$('#WiZConff').append(row);  
...
function gotoNATRuleEditPage(showinstancename,version)
{
	var instname = document.getElementById(showinstancename).value;
	var slnumber = document.getElementById("slno").value;
	location.href ="edit_natrules.jsp?instancename="+encodeURIComponent(instname)+"&slnumber="+slnumber+"&version="+version;
}
...
function addNATRule(showinstancename, addinstance,slnumber,version)
{
    var table = document.getElementById("WiZConff");
	var iprows = table.rows.length;
    if (iprows==26)
...
function deleteNATRulepage(id,slnumber,version)
{
    	var instname=document.getElementById(id).value;
		location.href = "savedetails.jsp?slnumber="+slnumber+"&page=natrules&instancename="+encodeURIComponent(instname)+"&version="+version+"&action=delete";
}

function validateDualIpForNat(id,checkempty,name,isCIDR)
	{
...
function CheckNatIPFormat(id,checkempty,name,isCIDR)
{
		var valid =validateIPOrNetwork(id,checkempty,name);
		if(!valid)
		{
...
function CheckDualIPFormat(id,checkempty,name,isCIDR)
{
