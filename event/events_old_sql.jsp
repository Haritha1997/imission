 ...
 if( user.getRole().equals(Role.ADMIN)||  user.getRole().equals(Role.SUPERADMIN) || !user.getRole().equals(Role.VIEW))  { 
     ...
     Session hibsession = null;	  
      try
      {
		  ...
		  String cntsql = "select count(*) from m2mlogs";
		  ...
		  if(fromdate.trim().length() > 0)
		  {
			  ...
			  sql += " updatetime >='"+DateTimeUtil.getNextTriggerTime(fromdate)+"'";
			  cntsql += " updatetime >='"+DateTimeUtil.getNextTriggerTime(fromdate)+"'";
		  }
		  if(todate.trim().length() > 0)
		  {
			  ...
			  sql += " updatetime <=(date('"+DateTimeUtil.getNextTriggerTime(todate)+"') + interval '1' day)";
			  cntsql += " updatetime <=(date('"+DateTimeUtil.getNextTriggerTime(todate)+"') + interval '1' day)";
		  }  
    	  ...
    	  while(rs.next())
    	  {
    		  ...
    		  res_sh.put("updatetime",sdf.format(rs.getTimestamp("updatetime")));
    		  res_sh.put("slnumber",rs.getString("slnumber"));
    		  res_sh.put("loginfo",rs.getString("loginfo"));
    		  res_vec.add(res_sh);
		  }
...
<table id="tab" class="table table-condensed severity">
	<thead id="sticky">
		<tr>
			<th width="10%"><%if(sortby.equals("severity")){%><img <%if(sorttype.equals("asc")){%>src="images/arrowup.gif"<%}else{%> src="images/arrowdown.gif"<%}%>alt="Severity Sort" vspace="0" hspace="0" border="0"/><%}%><label class="pointer" onclick="makelink('severity','<%=sorttype%>','<%=sortby%>')">Severity</label></th>
			<th width="19%"><%if(sortby.equals("updatetime")){%><img <%if(sorttype.equals("asc")){%>src="images/arrowup.gif"<%}else{%> src="images/arrowdown.gif"<%}%>alt="Severity Sort" vspace="0" hspace="0" border="0"/><%}%><label class="pointer" onclick="makelink('updatetime','<%=sorttype%>','<%=sortby%>')">Date</label></th>
			<th width="19%"><%if(sortby.equals("slnumber")){%><img <%if(sorttype.equals("asc")){%>src="images/arrowup.gif"<%}else{%> src="images/arrowdown.gif"<%}%>alt="Severity Sort" vspace="0" hspace="0" border="0"/><%}%><label class="pointer" onclick="makelink('slnumber','<%=sorttype%>','<%=sortby%>')">Serial No</label></th>
			<th width="50%"><%if(sortby.equals("loginfo")){%><img <%if(sorttype.equals("asc")){%>src="images/arrowup.gif"<%}else{%> src="images/arrowdown.gif"<%}%>alt="Severity Sort" vspace="0" hspace="0" border="0"/><%}%><label class="pointer" onclick="makelink('loginfo','<%=sorttype%>','<%=sortby%>')">Event</label></th>
	...
	%>
         ...
         <tr valign="top" class="severity-<%=res_sh.get("severity")%>">
			...
			<%=res_sh.get("severity").equals("normal")?"Normal":"Warning"%>
         	</td>
         	<td >
         	<%=res_sh.get("updatetime")%>
         	</td>
         	<td >
         	<%=res_sh.get("slnumber")%>
         	</td>
