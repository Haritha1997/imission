...
#devinfotab td,#configtab td, #ethporttab td, #celllartab td, #activitytab td, #wantab td, #diotab td,#zerotiretab td, #openvpntab td,#ipsectab td,#cpu_utilizatintab td
{
...
#devinfotab td:nth-child(odd),#celllartab td:nth-child(odd),#activitytab td:nth-child(odd),#configtab td:nth-child(odd),#cpu_utilizatintab td:nth-child(odd)
{
	min-width:40%;
	max-width:48%;
	width:40%;
...
#devinfotab td:nth-child(even),#celllartab td:nth-child(even),#activitytab td:nth-child(even),#configtab td:nth-child(even),#cpu_utilizatintab td:nth-child(even)
{
	min-width:52%;
	max-width:52%;
	width:52%;
...
/*  function hideorshow(divname)
{
	try{
	var sysobj = document.getElementById("systemdiv");
	var ntwrkobj = document.getElementById("ntwkdiv");
	 var configobj = document.getElementById("configdiv"); 
	var vpnobj = document.getElementById("ipsecdiv");
	var actobj = document.getElementById("activityConfigdiv");
	...
	else if(divname == "ntwkdiv")
	{
		vpnobj.style.display = "none";
		sysobj.style.display = "none";
		//configobj.style.display = "none";
		actobj.style.display = "none";
		ntwrkobj.style.display = "";
		dataobj.style.display = "none";
		alarmobj.style.display = "none";
	}
	...
	else if(divname == "configdiv")
	{
		configobj.style.display = "";
		vpnobj.style.display = "none";
		sysobj.style.display = "none";
		ntwrkobj.style.display = "none";
		actobj.style.display = "none";
...
try {
	...
	if (nd != null) {
		...
		if (version.startsWith(Symbols.WiZV2) || version.endsWith(Symbols.WiZV2)) {
			...
			device_info_ht.put("Load Average",nd.getLoadAverage()==null?"-":nd.getLoadAverage());
			devinfo_key_list.add("Load Average");
			device_info_ht.put("Total Usable Memory(RAM)",nd.getTotalUsableMemoryRAM()==null?"-":nd.getTotalUsableMemoryRAM());
			devinfo_key_list.add("Total Usable Memory(RAM)");
			device_info_ht.put("Used Memory",nd.getUsedMemory()==null?"-":nd.getUsedMemory());
			devinfo_key_list.add("Used Memory");
			device_info_ht.put("Free Memory(Unallocated)",nd.getUnallocatedFreeMemory()==null?"-":nd.getUnallocatedFreeMemory());
			devinfo_key_list.add("Free Memory(Unallocated)");
			device_info_ht.put("Memory Available(For Allocation)",nd.getMemoryAvailableForAllocation()==null?"-":nd.getMemoryAvailableForAllocation());
			devinfo_key_list.add("Memory Available(For Allocation)");
			device_info_ht.put("Cached",nd.getCached()==null?"-":nd.getCached());
			devinfo_key_list.add("Cached");
...
<a><img id="systemid" title="VPN" src="/imission/images/device-info.png" style="width: 25%;height:85%"></img>
System</a>
</li>
<li onclick="setPosition('ntwkdiv')">
<a style="padding-left:1%"><img id="ntwkid" title="VPN" src="/imission/images/interface.png" style="width: 25%;height:85%"></img>
Network</a>
</li>
<li onclick="setPosition('ipsecdiv')">
<a ><img id="vpnid" title="VPN" src="/imission/images/vpn.jpg" style="width: 25%;height:85%"></img>
VPN</a>
</li>
<li onclick="hideorshow('configdiv','<%=version%>')">
<a><img id="configid" title="Recent Alarms" src="/imission/images/config.png" style="width: 20%;height:80%"></img>
Configuration</a>
</li>
<li onclick="setPosition('activityConfigdiv')">
<a><img id="actid" title="Recent Activity" src="/imission/images/activity.jpg" style="width: 25%;height:85%"></img>
Recent Activity</a>
</li>
<li onclick="setPosition('datadiv')">
<a><img id="dataid" title="Data Usage" src="/imission/images/data-usage.png" style="width: 25%;height:85%"></img>
Data Usage</a>
</li>
<li onclick="setPosition('alarmsdiv')">
<a><img id="alarmsid" title="Recent Alarms" src="/imission/images/alarm.jpg" style="width: 25%;height:85%"></img>
Recent Alarms</a>
</li>
</ul>
</div> --%>
...
<div class="titleheader">Device Information</div>
<br>
...
<td colspan="2" width="35%" style="max-width: 35%">
<!-- code haritha -->
<%if(version.contains(Symbols.WiZV2)){%>
	...
	<%} %>
	<br>
	<table class="innertab" id="ethporttab" style="width:<%if(version.startsWith(Symbols.WiZV2+Symbols.ES)){%>60px<%}else {%>60%<%}%>">
	<tr>
	<%
	ethport_list.add("P0");
	ethport_list.add("P1");
...
<br>
<table class="innertab" id="wantab" style="width:80%">
<thead>
<tr>
...
<td style="min-width:30%">
<div class="titleheader toppaddiv" style="padding-left:45%">Availability</div>
<div class="titleheader" align="center" style="font-size: 14px;padding-left:20%;margin-top: 5%"><select id="outageopt" onchange="changeOutageChartData()"><option value="Today">Today</option><option value="Yesterday">Yesterday</option><option value="Lastweek">Last Week</option></select></div>
...
<div id="ntwkdiv"> -->
<%if(version.contains(Symbols.WiZV2)){%>
<div class="titleheader">CPU Load Average</div>
<br>
<table class="borderlesstab">
<tr>
<td style="min-width: 30%;">
<table class="innertab" id="cpu_utilizatintab">
<%for(int i=13;i<devinfo_key_list.size();i++) {%>
<tr>
<td><%=devinfo_key_list.get(i)%></td>
<td><%=device_info_ht.get(devinfo_key_list.get(i))==null?"-":device_info_ht.get(devinfo_key_list.get(i))%>
</td>
</tr>
<%} %>
</table>
</td>
<td colspan="2" width="35%" style="max-width: 35%"></td>
<td style="min-width:30%"></td>
</tr>
</table>
<%} %>

<div class="titleheader">Cellular</div>
<br>
<table class="borderlesstab">
<tr>
<td style="min-width:30%;">
<table class="innertab" id="celllartab">
...
<div id="ipsecdiv"> -->
<%if(version.contains(Symbols.WiZV2)){%>
<div class="titleheader">ZeroTier</div>
<br>
...
<!-- </div>
<div id="activityConfigdiv"> -->
<table class="borderlesstab">
<tr>
<td style="min-width: 30%">
<div class="titleheader toppaddiv">Activity</div>
<br>
<table class="innertab" id="activitytab">
...
<tr>
<td style="min-width: 30%">
<div class="titleheader ">Activity Status</div>
<br>
<table class="innertab" id="configtab">
...
String confsta = config_status.equals("Success")?"UP":config_status.equals("Fail")?"DOWN":config_status.equals("In Progress")?
		"Progress":config_status.equals("Updating")?"warning":config_status.equals("FW in Queue")?"DISABLED":"";
%>
<tr>
<td><%=config_key_list.get(i)%></td>
<td>
<label class="circle <%=confsta%>" style="margin-left: 3%;margin-right: 2%"></label>
<%=config_info_ht.get(config_key_list.get(i))==null?"-":(config_info_ht.get(config_key_list.get(i)).trim().length() > 0?config_info_ht.get(config_key_list.get(i)):"-")%>
</td>
<%} %>
</table>
<!-- </div> -->
...
<table class="innertab" id="sim1charttable" style="width:80%;">
<tr>
...
<div class="titleheader " align="center" id="sim2charttable">SIM2</div>
<table class="innertab" id="sim2Charttable" style="width:80%;">
<tr>
<td width="33%" style="max-width:33%">
...
<div id="alarmsdiv">
<div class="titleheader ">Recent Alarms</div>
<br>
